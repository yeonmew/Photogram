...
function toggleSubscribe(toUserId, obj) {
    if ($(obj).text() === "구독취소") {
        ...
        $.ajax({
            type: "delete",
            url: "/api/subscribe/${pageUserId}" + toUserId,
            dataType: "json"
        }).done(res => {
            $(obj).text("구독하기");
...
function getSubscribeModalItem(u) {
    ...
    console.log(!u.equalUserState); // 나랑 같으면 1, 다르면 0
    console.log(u.subscribeState); // 내가 구독을 했으면 1, 다르면 0
    if (!u.equalUserState) { //동일 유저가 아닐 때 버튼이 만들어진다.
